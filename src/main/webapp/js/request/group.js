...
var group = {
    ...
    "url": {
        ...
        "randomChangeGroupIcon": function () {
            return "/gsdp/group/admin/randomChangeGroupIcon";
        },

        "changeGroupIcon": function () {
            return "/gsdp/group/admin/ChangeGroupIcon";
        },

        "changeGroupInfo": function () {
            return "/gsdp/group/admin/ChangeGroupInfo";
        }
    },
    ...
    "showModifyGroupInfoDialog": function () {
        dialog.showDialog(parseInt($(".modify-group-info-size").css("min-height")),
            $(".modify-group-info-size").outerWidth(), "modify-group-info-dialog");
    },
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    for (var i = 0; i < data.data.members.length; ++i) {
                        ...
                        $("#group-apply-member-manager-table tbody tr:eq(" + i + ") td:eq(4)").attr("user-id", data.data.members[i].userId).html("<button type='button' title='同意' class='btn btn-info btn-sm'>√</button>" +
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    for (var i = 0; i < data.data.groupMember.members.length; ++i) {
                        ...
                        $("#group-member-manager-table tr:eq(" + i + ") td:eq(0)").html("<img src='/gsdp/" + data.data.groupMember.members[i].user.headPicture + "'/>&nbsp;" +
                            data.data.groupMember.members[i].user.username);

                        $("#group-member-manager-table tr:eq(" + i + ") td:eq(1)").html(user.reference.genderReference()[data.data.groupMember.members[i].user.sex][1]);

                        $("#group-member-manager-table tr:eq(" + i + ") td:eq(2)").html(data.data.groupMember.members[i].user.age + "岁");

                        $("#group-member-manager-table tr:eq(" + i + ") td:eq(3)").html(data.data.groupMember.members[i].role.roleName);
    ...
    "randomChangeGroupIcon": function () {
        var groupId = $("#groupId").text();
        $.post(group.url.randomChangeGroupIcon(),
            {
                "groupId" : groupId
            }, function (data) {
                if (data.success) {
                    $(".modify-group-icon > img").attr("src", "/gsdp/" + data.data);
                } else {
                    alert(data.message);
                }
            }, "json");
    ...
    "changeGroupIcon": function () {
        ...
        if (groupIcon) {
            $.ajaxFileUpload({
                type: "post",
                url: group.url.changeGroupIcon(),
                secureuri: false,
                fileElementId: "groupIcon",
                dataType: "json",
                data: {
                    "groupId" : groupId
                },
    ...
    "changeGroupInfo": function () {
        var groupId = $("#groupId").text();
        var groupName = $("#modify-group-name").val();
        var groupContact = $("#modify-group-contact").val();
        var groupAddress = $("#modify-group-address").val();
        var groupType = $("#modify-group-type").val();
        var groupDec = $("#modify-group-introduce").val();

        if (group.check.checkGroupName(groupName) && group.check.checkGroupContact(groupContact) &&
            group.check.checkGroupAddress(groupAddress) && group.check.checkGroupDec(groupDec)) {
            $.post(group.url.changeGroupInfo(),
                {
                    "groupId": groupId,
                    "groupName": groupName,
                    "groupContact": groupContact,
...
$(function () {
    //实现用户输入团队名称的提示
    $("#group_name,#modify-group-name").on("blur", function () {
...
//实现用户输入电话号码的提示
    $("#create-group-contact,#modify-group-contact").on("blur", function () {
    ...
    //实现用户输入办公地址的提示
    $("#work_address,#modify-group-address").on("blur", function () {
    ...
    //实现用户输入团队介绍的提示
    $("#group_introduce,#modify-group-introduce").on("blur", function () {
    ...
    //随机修改团队头像
    $("#modify-group-info-form .change-picture").on("click", function () {
        group.randomChangeGroupIcon();
    });

    //用户上传本地的图片到团队头像
    $("#modify-group-info-form").on("change", "#groupIcon", function () {
        group.changeGroupIcon();
    });

    //用户点击修改团队资料
    $("#modify-group-info-dialog .modal-footer button:eq(0)").on("click", function () {
